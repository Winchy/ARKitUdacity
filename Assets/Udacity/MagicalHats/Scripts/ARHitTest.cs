...
public class ARHitTest : MonoBehaviour {
	...
	IEnumerator ShuffleTime(int numSuffles) {
        ...
        for (int i = 0; i < numSuffles; ++i)
        {
            ...
            spawnedObjects.Remove(randFrom);
            GameObject randTo = spawnedObjects[Random.Range(0, spawnedObjects.Count)];
            spawnedObjects.Add(randFrom);
            yield return StartCoroutine(Swap(randFrom.transform, randTo.transform, 0.5f));
        }
	}
	...
	IEnumerator Swap(Transform item1, Transform item2, float duration){
        ...
        {
            t += Time.deltaTime;
            item1.position = Vector3.Lerp(pos1, pos2, t / duration);
            item2.position = Vector3.Lerp(pos2, pos1, t / duration);
            yield return null;
        }
	}
}